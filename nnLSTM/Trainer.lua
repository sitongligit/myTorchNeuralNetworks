...
--- Trainer is a general and class to train nn or nngraph neural network models.
...
-- @param struct with two fields. nnet: nn o nngraph model, criterion: any torch nn criterion.
-- @param loader module to load each batch. Should provide the functions *nextTrain()* and *nextValidation()*
function Trainer:__init(model, loader)
    self.model = model
    self.loader = loader
end
...
function Trainer:train()
    ...
    print('      > Total number of params: '.. model.nnet:getParameters():size(1))
    ...
    local function feval(parameters)
        ...
        -- forward pass
        output = model.nnet:forward(input)

        -- forward through the criterion
        loss = model.criterion:forward(output, y)

        -- loss and soft-max layer backward pass
        dloss = model.criterion:backward(output, y)
        model.nnet:backward(input, dloss)
    ...
    for i = 1, iterations do
        ...
        if i % opt.save_every == 0 or i == iterations then
            local val_err = validate(model, loader, true)
            saveModel(model, 1-val_err, epoch)
        end        
...
--- Validation function.
-- @param draw Boolean value to switch the plotting of the prediction and the targets
...
function saveModel(model,acc,epoch)
...
function main()
    ...
    Loader = require '../utils/BTCLoader'
    loader = Loader.new(opt.batch_size, opt.time_steps, opt.train_file)
...
-- load model
-- local checkpoint = torch.load('checkpoints/cnn_lstm_net_epoch=77_acc=0.9999.t7')
-- model = checkpoint.model
-- opt = checkpoint.opt

-- -- get the loader
-- Loader = require '../utils/BTCLoader'
-- loader = Loader.new(opt.batch_size, opt.time_steps, opt.data_dir..'/'..opt.train_file)

-- -- validate(model, loader, true)
-- for j = 1, loader.validation_size / opt.batch_size do

--     local x,y = loader:nextTrain()
--     local prediction = model.model:forward(x)

--     for i = 1,prediction:size(1) do
--         print(string.format('GT: %0.3f ----> prediction: %0.3f',y[i]:squeeze(),prediction[i]:squeeze()))
--     end    
-- end
